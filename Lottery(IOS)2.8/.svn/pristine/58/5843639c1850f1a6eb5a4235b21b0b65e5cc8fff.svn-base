...
- (void)initValues
{
    ...
    //创建顶部可滑动的tab
    _topScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, _tabHeight)];
    ...
    _tabBackgroundImageView = [[UIImageView alloc] initWithFrame:_topScrollView.bounds];
    [_topScrollView addSubview:_tabBackgroundImageView];
    
    //创建主滚动视图
    _rootScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, _tabHeight, self.bounds.size.width, self.bounds.size.height - _tabHeight)];
    ...
    _nameBtns =  [[NSMutableArray alloc] init];
    _isBuildUI = NO;
...
- (void)setTabBackgroundImage:(UIImage *)tabBackgroundImage
{
    _tabBackgroundImage = tabBackgroundImage;
    _tabBackgroundImageView.image = _tabBackgroundImage;
    
    _tabHeight = _tabBackgroundImage.size.height;
}
    ...
    if (_isBuildUI) {
        ...
        if (self.rigthSideButton.bounds.size.width > 0) {
            ...
            _topScrollView.frame = CGRectMake(0, 0,
                                              self.bounds.size.width - self.rigthSideButton.bounds.size.width, _tabHeight);
        }
...
- (void)buildUI
{
    CGRect tRect = _topScrollView.frame;
    tRect.size.height = _tabHeight;
    _topScrollView.frame = tRect;
    
    CGRect rRect = _rootScrollView.frame;
    rRect.origin.y = _tabHeight;
    rRect.size.height = self.bounds.size.height - _tabHeight;
    _rootScrollView.frame = rRect;
...
- (void)createNameButtons
{
    ...
    [_indicatorImageView setImage:_indicatorImage];
    [_topScrollView addSubview:_indicatorImageView];
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        CGSize textSize = [vc.title sizeWithFont:[UIFont systemFontOfSize:kFontSizeOfTabButton]
                               constrainedToSize:CGSizeMake(_topScrollView.bounds.size.width, _tabHeight)
                                   lineBreakMode:NSLineBreakByTruncatingTail];
        //累计每个tab文字的长度
        topScrollViewContentWidth += kWidthOfButtonMargin+textSize.width;
        //设置按钮尺寸
        [button setFrame:CGRectMake(xOffset,0,
                                    textSize.width, _tabHeight)];
        //计算下一个tab的x偏移量
        xOffset += textSize.width + kWidthOfButtonMargin;
        
        [button setTag:i+100];
        if (i == 0) {
            _indicatorImageView.frame = CGRectMake(kWidthOfButtonMargin,
                                                _tabHeight-2,
                                                textSize.width,
    ...
    //设置顶部滚动视图的内容总尺寸
    _topScrollView.contentSize = CGSizeMake(topScrollViewContentWidth, _tabHeight);
}
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            [_indicatorImageView setFrame:CGRectMake(sender.frame.origin.x,
                                                  _indicatorImageView.frame.origin.y,//0,
            ...
            if (finished) {
                ...
                if (self.slideSwitchViewDelegate && [self.slideSwitchViewDelegate respondsToSelector:@selector(slideSwitchView:didSelectTab:)]) {
                    _selectedIndex = _userSelectedChannelID - 100;
                    [self.slideSwitchViewDelegate slideSwitchView:self didSelectTab:_selectedIndex];
                }
            }
        }];
