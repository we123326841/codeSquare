//
//  AgentManualSettingController.m
//  Caipiao
//
...
#import "NewLink.h"
@interface AgentManualSettingController()<UITableViewDelegate>
@property (retain, nonatomic) IBOutletCollection(UIButton) NSArray *btns;
@property (retain, nonatomic) IBOutlet UIButton *guiLinBtn;
...
@implementation AgentManualSettingController
...
-(void)switchModel:(RadioButtonType)type{
   ...
   // NSString *path=[docPath stringByAppendingPathComponent:@"agentType"];
    
    // NSArray *p=[NSKeyedUnarchiver unarchiveObjectWithFile:path];
    RQManualSetting *rq = (RQManualSetting*)self.rq;
    //rq.objects =[rq.copyObjects copy];
  //  rq.objects=p;
...
-(void)requestData
{
    ...
    rq.type=@1;
    self.rq = rq;
    __block AgentManualSettingController *blockself = self;
   ...
   // _isSelected=NO;
    ...
    [rq.objects enumerateObjectsUsingBlock:^(NSArray* obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [obj enumerateObjectsUsingBlock:^(NSArray*awardList, NSUInteger idx, BOOL * _Nonnull stop) {
            [awardList enumerateObjectsUsingBlock:^(NSArray *award, NSUInteger idx, BOOL * _Nonnull stop) {
//                [award enumerateObjectsUsingBlock:^(UserAwardList*o, NSUInteger idx, BOOL * _Nonnull stop) {
//                    if (o.isSelected&&![o.directRetReal isEqualToString:@"未设置"]) {
//                        _isSelected =YES;
//                        return ;
//                        
//                    }
