...
- (void) dealloc
{
    ...
    [[self class] cancelPreviousPerformRequestsWithTarget:self selector:@selector(showHUD:) object:@(_hiddenHUD)];
...
//    _topHUD.backgroundColor=[UIColor redColor];
    _topBar    = [[UIToolbar alloc] initWithFrame:CGRectMake(0, 0, width, topH)];
//    _topBar.backgroundColor = [UIColor greenColor];
...
//    [self.view addSubview:_topHUD];
    ...
    _doneButton.frame = CGRectMake(10, 1, 50, topH);
...
//    [_doneButton setTitle:NSLocalizedString(@"OK", nil) forState:UIControlStateNormal];
    [_doneButton setImage:ResImage(@"video_close.png") forState:UIControlStateNormal];
    _doneButton.imageEdgeInsets=UIEdgeInsetsMake(14, 14, 14, 14);
...
//                          forState:UIControlStateNormal];
    ...
    playBtn.frame = CGRectMake(0, 0, 50, topH);
    [playBtn setImage:[UIImage imageNamed:@"kxmovie.bundle/playback_play.png"] forState:UIControlStateNormal];
    playBtn.imageEdgeInsets=UIEdgeInsetsMake(10, 13, 10, 13);
    [playBtn addTarget:self action:@selector(playDidTouch:)
          forControlEvents:UIControlEventTouchUpInside];

    _playBtn = [[UIBarButtonItem alloc] initWithCustomView:playBtn];
    _playBtn.width = 50;
    
    
    UIButton* pauseBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    pauseBtn.frame = CGRectMake(0, 0, 50, topH);
    [pauseBtn setImage:[UIImage imageNamed:@"kxmovie.bundle/playback_pause.png"] forState:UIControlStateNormal];
    pauseBtn.imageEdgeInsets=UIEdgeInsetsMake(10, 13, 10, 13);
    [pauseBtn addTarget:self action:@selector(playDidTouch:)
      forControlEvents:UIControlEventTouchUpInside];

    _pauseBtn = [[UIBarButtonItem alloc] initWithCustomView:pauseBtn];
       _pauseBtn.width = 50;

    _loadingBtn = [[UIButton alloc]init];
    _loadingBtn.frame = CGRectMake(0, 0, 50, topH);
    [_loadingBtn setImage:[UIImage imageNamed:@"kxmovie.bundle/playback_loading.png"] forState:UIControlStateNormal];
    _loadingBtn.imageEdgeInsets=UIEdgeInsetsMake(9,9,9, 9);
    [_bottomBar addSubview:_loadingBtn];
    _loadingBtn.center=CGPointMake([UIScreen mainScreen].bounds.size.height/2, 25);
    ...
    [_loadingBtn.layer addAnimation:rotationAnimation forKey:@"Rotation"];
    
    UIButton *screenBtn = [[UIButton alloc]init];
    screenBtn.frame = CGRectMake(0, 0, 50, topH);
    [screenBtn setImage:[UIImage imageNamed:@"kxmovie.bundle/video_screen.png"] forState:UIControlStateNormal];
    screenBtn.imageEdgeInsets=UIEdgeInsetsMake(11,11,11, 11);
    [_bottomBar addSubview:screenBtn];
    screenBtn.center=CGPointMake([UIScreen mainScreen].bounds.size.height-25, 25);
    [screenBtn addTarget:self action:@selector(screenBtnClicked:)
       forControlEvents:UIControlEventTouchUpInside];
...
-(void)hiddenBottomBarAfter5Sceonds
{
    [[self class] cancelPreviousPerformRequestsWithTarget:self selector:@selector(showHUD:) object:@(_hiddenHUD)];
    [self performSelector:@selector(showHUD:) withObject:@(_hiddenHUD) afterDelay:5];
}
- (void) handleTap: (UITapGestureRecognizer *) sender
{
    if (sender.state == UIGestureRecognizerStateEnded) {
        ...
        if (sender == _tapGestureRecognizer) {
            ...
            [self hiddenBottomBarAfter5Sceonds];
...
- (void) updateBottomBar:(BOOL)waiting
{
    if (waiting) {
        _loadingBtn.hidden=NO;
        [_bottomBar setItems:@[_spaceItem, _fixedSpaceItem,
                               _fixedSpaceItem, _spaceItem] animated:NO];
    }else{
        _loadingBtn.hidden=YES;
    UIBarButtonItem *playPauseBtn = self.playing ? _pauseBtn : _playBtn;
    [_bottomBar setItems:@[_spaceItem, _fixedSpaceItem, playPauseBtn,
                           _fixedSpaceItem, _spaceItem] animated:NO];
    }
